...
def run_linear(
    ...
    _ = ll.load_state_dict(state_dict=sd)
...
def run_embedding(
    ...
    _ = emb.load_state_dict(state_dict=sd)
...
def run_multihead_self_attention(
    ...
    cmha_l = CausalMultiHeadSelfAttention(d_model, num_heads)
    cmha_l.load_state_dict(sd)

    return cmha_l.forward(in_features)
...
def run_rmsnorm(
    ...
    """
    sd = {"G": weights}
    rmsl = RMSNorm(d_model, eps)
    _ = rmsl.load_state_dict(sd)

    return rmsl.forward(in_features)
...
def run_get_batch(
    ...
    """
    return get_input_target_pairs(dataset, batch_size, context_length, device_str=device)
